...
public class MainViewModel extends BaseRxViewModel {
    ...
    private void executeRequest(FilterableRequest request) {
        ...
        Disposable disposable = RXKt.asSingle(request).subscribe(
                searchResults -> onResult(request, searchResults),
                throwable -> MainViewModel.this.errorLiveData.postValue(throwable.toString()));
    ...
    private void onResult(FilterableRequest request, SearchResults searchResults) {
        ...
        MainViewModel.this.stateLiveData.postValue(state);
    }
